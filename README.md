# Shahid Falah 👨‍💻
`Full-stack developer` | `4th-year CS student` | `Based in Halabja, Kurdistan 🇮🇶`

## 👋 About Me

Builder first, student second. I don't just study technology — I ship it.

Currently building **[Prompt Saz](https://github.com/shahidfalah)** — a multilingual AI prompt discovery platform for Kurdish and Arabic speakers, and **DermanMap** — a web app tailored for pharmacies and labs across Iraq.

4th-year Computer Science student at the University of Halabja. I've gone from WordPress themes to React to Next.js + Supabase, learning by building real products the whole way. Interested in where UI/UX, AI, and underserved communities intersect.

Languages I speak: Kurdish 🇮🇶 · Arabic · English

## ⚙️ Tech Stack & Tools
<br/>

### Programming Languages:
<code><img title="JavaScript" alt="JavaScript" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="42px"/></code>
<code><img title="TypeScript" alt="TypeScript" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="42px"/></code>
<code><img title="Python" alt="Python" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="42px"/></code>
<code><img title="Java" alt="Java" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="42px"/></code>
<br/>

### Frontend:
<code><img title="React" alt="React" src="https://raw.githubusercontent.com/github/explore/main/topics/react/react.png" height="42px"/></code>
<code><img title="Next.js" alt="Nextjs" src="https://raw.githubusercontent.com/github/explore/main/topics/nextjs/nextjs.png" height="42px"/></code>
<code><img title="Tailwind CSS" alt="Tailwind CSS" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" height="42px"/></code>
<code><img title="HTML5" alt="HTML5" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="42px"/></code>
<code><img title="CSS3" alt="CSS3" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="42px"/></code>
<code><img title="Figma" alt="Figma" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" height="42px"/></code>
<br/>

### Databases & Backend:
<code><img title="Supabase" alt="Supabase" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/supabase/supabase-original.svg" height="42px"/></code>
<br/>

### Development & DevOps:
<code><img title="Git" alt="Git" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="42px"/></code>
<code><img title="GitHub" alt="GitHub" src="https://images.icon-icons.com/3685/PNG/512/github_logo_icon_229278.png" height="42px"/></code>
<code><img title="Postman" alt="Postman" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postman/postman-original.svg" height="42px"/></code>
<code><img title="Vercel" alt="Vercel" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vercel/vercel-original.svg" height="42px"/></code>
<code><img title="Lighthouse" alt="Lighthouse" src="https://tehnoblog.org/wp-content/uploads/2019/01/Google-Chrome-Lighthouse-Logo.png" height="42px"/></code>
<br/>

## 🚀 What I'm Building

| Project | Description | Stack |
|---|---|---|
| **Prompt Saz** | Multilingual AI prompt gallery for Kurdish & Arabic speakers | Next.js · Supabase |
| **DermanMap** | Web platform for pharmacies and labs in Iraq | Next.js · Supabase |

## 📈 My Journey

<p align="center">
<img height="50%" width="auto" src="https://github-readme-stats-steel-theta-71.vercel.app/api?username=shahidfalah&show_icons=true&include_all_commits=true&theme=tokyonight&hide_border=true&hide=issues,contribs,stars&bg_color=00000000&show=prs_merged,prs_merged_percentage">
<img height="50%" width="auto" src="https://github-readme-stats-steel-theta-71.vercel.app/api/top-langs/?username=shahidfalah&layout=compact&hide_border=true&theme=tokyonight&bg_color=00000000&langs_count=6">
</p>

## 📍 Connect

Based in Halabja, Kurdistan Region, Iraq 🇮🇶 · Kurdish · Arabic · English 💬

📫 shahid.falah.j@gmail.com

> *"Build things that matter for people who are overlooked."*
